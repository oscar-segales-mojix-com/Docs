...
read -p 'PortExternal:' port
read -p 'user: ' user
read -p 'pass: ' pass
read -p 'Premise: ' premise
...
premID=$(curl -X GET 'http://'$server':'$port'/statemachine-api-configuration/rest/configuration/locations?level=premise' -H 'Authorization: Basic '$base'' | jq '.[] | select(.code=="'$premise'")'| jq -r '.id')
echo $premID

## POS GET DATA
POS_id=$(curl -X GET 'http://'$server':'$port'/statemachine-api-configuration/rest/locations/'$premID'/infra/devices?model=KEONN__ADPY__1&ts=1576156454337' -H 'Authorization: Basic '$base''| jq -r '.[0].id ' )
echo $POS_id

curl -X GET 'http://'$server':'$port'/statemachine-api-configuration/rest/infra/devices/'$POS_id'/povs' -H 'Authorization: Basic '$base''  > /tmp/jsPOS.json
...
## POE GET DATA
POE_id=$(curl -X GET 'http://'$server':'$port'/statemachine-api-configuration/rest/locations/'$premID'/infra/device/types/POE/devices?ts=1576167798640' -H 'Authorization: Basic '$base''| jq -r '.[0].id ' )
echo $POE_id

curl -X GET 'http://'$server':'$port'/statemachine-api-configuration/rest/infra/devices/'$POE_id'/povs' -H 'Authorization: Basic '$base''  > /tmp/jsPOE.json
